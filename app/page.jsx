    ...
    <div style={{ minHeight: "100vh" }}>
      {selectedNode && (
        <>
          ...
          <NodeDetailDrawer node={selectedNode} data={data} onClose={() => setSelectedNode(null)} />
        </>
      )}
      ...
      {!data ? (
        ...
        <div style={{ padding: "0 28px 40px" }}>
          ...
          {/* FIX: Flow + Field Mapping both receive selectedMapping */}
          {tab === "flow" && (
            <FlowCanvas data={data} onNodeClick={setSelectedNode} selectedMapping={selectedMapping} />
          )}
...
function Header({ fileName, hasData, onReset }) {
  return (
    <div style={{ padding: "20px 28px", borderBottom: "1px solid #1e1e2e", display: "flex", alignItems: "center", gap: 16, background: "linear-gradient(180deg, #0a0a12, #06060a)" }}>
      ...
      <div>
        <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: "-0.02em" }}>INFA Flow Visualizer</div>
        <div style={{ fontSize: 11, color: "#4a4a6a", fontFamily: FONT_MONO }}>PowerCenter XML → Visual Flow + Field Mapping</div>
      </div>
      {hasData && (
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 11, color: "#4a4a6a", fontFamily: FONT_MONO }}>{fileName}</span>
          <button onClick={onReset} style={{ padding: "6px 14px", background: "#1e1e2e", border: "1px solid #2a2a3e", borderRadius: 6, color: "#6b7280", fontSize: 11, cursor: "pointer", fontFamily: FONT_MONO }}>New File</button>
        </div>
      )}
    </div>
  );
...
function RepositoryInfo({ data }) {
  ...
  return (
    <div style={{ padding: "10px 0", fontSize: 11, color: "#4a4a6a", fontFamily: FONT_MONO }}>
...
function TabBar({ tabs, activeTab, onTabChange, mappings, selectedMapping, onMappingChange }) {
  return (
    <div style={{ display: "flex", gap: 2, marginBottom: 20, background: "#08080d", padding: 4, borderRadius: 10, border: "1px solid #1e1e2e", flexWrap: "wrap" }}>
      {tabs.map((t) => (
        <button
          key={t.id}
          ...
          style={{
            ...
            border: activeTab === t.id ? "1px solid #1e1e2e" : "1px solid transparent",
            borderRadius: 8, color: activeTab === t.id ? "#e2e8f0" : "#4a4a6a",
            fontSize: 11, fontWeight: 500, cursor: "pointer", fontFamily: FONT_MONO,
      ...
      {/* Mapping selector — only show when there are multiple mappings */}
      {mappings.length > 1 && (
        <select
          ...
          style={{
            ...
            borderRadius: 8, color: "#6b7280", fontSize: 11, fontFamily: FONT_MONO,
