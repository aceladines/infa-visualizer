...
export default function FlowCanvas({ data, onNodeClick, selectedMapping }) {
  // FIX: pass selectedMapping to buildFlowGraph so only relevant nodes appear
  ...
  // Auto-fit when layout changes (new data or mapping selection)
  useEffect(() => {
    fitToView();
  }, [layout]);
  ...
  const zoomIn = useCallback(() => setZoom((z) => Math.min(ZOOM_CONFIG.MAX, z + ZOOM_CONFIG.STEP)), []);
  const zoomOut = useCallback(() => setZoom((z) => Math.max(ZOOM_CONFIG.MIN, z - ZOOM_CONFIG.STEP)), []);
  ...
  if (graph.nodes.length === 0) {
    return (
      <div style={{
        padding: 60, textAlign: "center", color: "#4a4a6a",
        fontFamily: FONT_MONO, fontSize: 13,
        background: "#0a0a0f", borderRadius: 12, border: "1px solid #1e1e2e",
      }}>
        {selectedMapping && selectedMapping !== "all"
          ? `No flow data found for mapping "${selectedMapping}"`
          : "No mapping connectors found in this export"}
      </div>
    );
  }
  ...
  return (
    <div style={{ position: "relative" }}>
      <div
        ...
        style={{
          overflow: "hidden", borderRadius: 12, background: "#0a0a0f",
          border: "1px solid #1e1e2e", height: "70vh", minHeight: 400,
          cursor: isDragging ? "grabbing" : "grab", touchAction: "none", position: "relative",
        }}
      >
        <svg
          width={layout.totalW}
        ...
        >
          ...
          {graph.edges.map((e, i) => (
            <Edge key={`e-${i}`} edge={e} index={i} positions={layout.positions} isHighlighted={hoveredEdge === i || hoveredNode === e.from || hoveredNode === e.to} onHover={setHoveredEdge} />
          ))}
          {graph.nodes.map((n) => (
            <Node key={n.id} node={n} pos={layout.positions[n.id]} isHovered={hoveredNode === n.id} onHover={setHoveredNode} onClick={onNodeClick} />
          ))}
        </svg>
      </div>
      ...
      {/* Zoom indicator */}
      <div style={{ position: "absolute", top: 12, left: 12, fontSize: 10, fontFamily: FONT_MONO, color: "#4a4a6a", background: "#0a0a0fcc", padding: "4px 8px", borderRadius: 4, border: "1px solid #1e1e2e", zIndex: 10, userSelect: "none" }}>
        {Math.round(zoom * 100)}%
      ...
      <div style={{ position: "absolute", bottom: 12, right: 12, width: minimapW, height: minimapH, background: "#08080ddd", border: "1px solid #1e1e2e", borderRadius: 6, overflow: "hidden", zIndex: 10 }}>
        <svg width={minimapW} height={minimapH}>
          {graph.nodes.map((n) => {
            ...
            return <rect key={n.id} x={pos.x * mmScale} y={pos.y * mmScale} width={pos.w * mmScale} height={pos.h * mmScale} fill={colors.border} opacity={0.6} rx={1} />;
          })}
          {graph.edges.map((e, i) => {
            const from = layout.positions[e.from];
            const to = layout.positions[e.to];
            if (!from || !to) return null;
            return <line key={`me-${i}`} x1={(from.x + from.w) * mmScale} y1={(from.y + from.h / 2) * mmScale} x2={to.x * mmScale} y2={(to.y + to.h / 2) * mmScale} stroke="#2a2a3e" strokeWidth={0.5} />;
          })}
          {viewportRect && <rect x={viewportRect.x} y={viewportRect.y} width={viewportRect.w} height={viewportRect.h} fill="none" stroke="#60a5fa" strokeWidth={1} rx={1} />}
        </svg>
      </div>
    </div>
  );
...
function ZoomBtn({ onClick, title, small, children }) {
  return (
    <button onClick={onClick} title={title} style={{ width: 32, height: 32, background: "#0a0a0fcc", border: "1px solid #1e1e2e", borderRadius: 6, color: "#6b7280", fontSize: small ? 10 : 18, fontFamily: FONT_MONO, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}>
      {children}
    </button>
  );
}
...
function Defs() {
  return (
    <defs>
      <marker id="arrowhead" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#4a4a6a" /></marker>
      <marker id="arrowhead-hl" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#60a5fa" /></marker>
      <filter id="glow"><feGaussianBlur stdDeviation="3" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
      <pattern id="grid" width="24" height="24" patternUnits="userSpaceOnUse"><path d="M 24 0 L 0 0 0 24" fill="none" stroke="#12121f" strokeWidth="0.5" /></pattern>
    </defs>
  );
}
