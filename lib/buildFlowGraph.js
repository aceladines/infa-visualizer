...
/**
 * Build a directed graph of nodes and edges from parsed data.
 *
 * Key fixes:
 * - Accepts selectedMapping to filter to a single mapping (or "all")
 * - Workflow tasks are NO LONGER mixed into the mapping flow graph.
 *   They live in the Workflows tab only. This prevents disconnected
 *   clusters piling up at layer 0 and creating a messy diagram.
 * - Mapplet-aware node classification.
 *
 * @param {object} data - Parsed XML result
 * @param {string} [selectedMapping="all"] - Filter to specific mapping name
...
export function buildFlowGraph(data, selectedMapping = "all") {
  ...
  // Instance types from mapplet references
  const mappletInstanceNames = new Set();
  data.mappings.forEach((m) => {
    (m.mappletInstances || []).forEach((mi) => {
      mappletInstanceNames.add(mi.name);
      if (mi.transformationName) mappletInstanceNames.add(mi.transformationName);
    });
  });
  ...
  const xformMeta = {};
  data.transformations.forEach((x) => { xformMeta[x.name] = x; });
  ...
  function classifyNode(name, instanceType) {
    ...
    if ((instanceType || "").toLowerCase() === "mapplet") return "mapplet";
    return "transform";
  }

  function getNodeMeta(name) {
    return sourceMeta[name] || targetMeta[name] || xformMeta[name] || mappletMeta[name] || shortcutMeta[name] || null;
  }
  ...
  mappingsToProcess.forEach((m) => {
    // Register mapping-level transforms in metadata lookup
  ...
  // NOTE: Workflow tasks are NOT added here anymore.
  // They caused disconnected clusters that made the diagram messy.
  // Workflow execution order is shown in the dedicated Workflows tab.

  return { nodes: Object.values(nodeMap), edges };
}

/**
 * Build a separate graph for workflow execution order (used by Workflows tab if needed).
 */
  ...
  workflow.tasks.forEach((t) => {
    if (!nodeMap[t.name]) {
      nodeMap[t.name] = {
        ...
        meta: { name: t.name, type: t.type, description: "" },
      };
    }
  });

  workflow.links.forEach((l) => {
