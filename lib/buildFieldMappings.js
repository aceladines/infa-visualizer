...
export function buildFieldMappings(data) {
  ...
  data.mappings.forEach((mapping) => {
    ...
    function getReachableOutputPorts(instanceName, inputField) {
      ...
      // ── Joiner: same-name pass-through only (like Sorter) ──
  ...
  // ── Post-processing: reject suspicious fan-out mappings ──
  // If a single source field maps to many unrelated target fields through the
  // same transformation path with no per-field expression evidence, reclassify
  // as NOT_CARRIED_BY_BRANCH with LOW confidence.
  ...
  for (const [, group] of fanOutGroups) {
    ...
    const hasPerFieldEvidence = group.some((m) =>
      (m.transformationSteps || []).some((s) => {
        const expr = (s.expression || "").trim();
        return expr && !/^[A-Za-z_][A-Za-z0-9_]*$/.test(expr);
      })
    );
    ...
    if (!hasPerFieldEvidence) {
      for (const m of group) {
        if (m.sourceField !== m.targetField) {
          m.lineageType = "NOT_CARRIED_BY_BRANCH";
          m.confidence = "LOW";
          m.evidenceChain += " [rejected: fan-out without per-field evidence]";
        }
      }
    }
  }
