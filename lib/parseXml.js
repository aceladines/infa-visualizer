...
function parseWorkflow(w) {
  ...
  // Deduplicate TASK and TASKINSTANCE by name — TASKINSTANCE takes priority
  // since it carries execution-specific attributes (TRANSFORMATION_NAME, FAIL_PARENT_IF_INSTANCE_FAILS).
  ...
  w.querySelectorAll("TASKINSTANCE").forEach((ti) => {
    ...
    taskMap.set(name, {
      name,
      type: ti.getAttribute("TASKTYPE") || existing?.type || "",
  ...
  wf.tasks = Array.from(taskMap.values());
...
function parseTransformation(tElement) {
  ...
  // Parse FIELDDEPENDENCY elements for custom/union transformations
  const fieldDependencies = [];
  tElement.querySelectorAll(":scope > FIELDDEPENDENCY").forEach((fd) => {
    fieldDependencies.push({
      inputField: fd.getAttribute("INPUT_FIELD") || "",
      outputField: fd.getAttribute("OUTPUT_FIELD") || "",
      inputGroup: fd.getAttribute("INPUT_GROUP") || "",
      outputGroup: fd.getAttribute("OUTPUT_GROUP") || "",
    });
  });
  ...
  return {
    ...
    ...(fieldDependencies.length > 0 ? { fieldDependencies } : {}),
  };
}
