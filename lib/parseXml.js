...
export function parseInformaticaXML(xmlString) {
  ...
  doc.querySelectorAll("MAPPING").forEach((m) => {
    result.mappings.push(parseMappingOrMapplet(m, result, "MAPPING"));
  });

  // ── Mapplets (reusable transformation groups — same structure as mappings) ──
  doc.querySelectorAll("MAPPLET").forEach((ml) => {
    const mapplet = parseMappingOrMapplet(ml, result, "MAPPLET");
    result.mapplets.push(mapplet);
    // Mapplets also contribute to the mapping list for flow/connector tracing
    result.mappings.push(mapplet);
  });
...
function parseMappingOrMapplet(element, result, kind) {
  const obj = {
  ...
  element.querySelectorAll("INSTANCE").forEach((inst) => {
    ...
    // Track mapplet instances specifically (TYPE="Mapplet")
    if (
  ...
  element.querySelectorAll("CONNECTOR").forEach((c) => {
